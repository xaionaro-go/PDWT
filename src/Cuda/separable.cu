...
int w_compute_filters_separable(const char* wname, int do_swt) {
  ...
  if (!do_swt) {
    if ((!strcasecmp(wname, "haar")) || (!strcasecmp(wname, "db1")) ||
        (!strcasecmp(wname, "bior1.1")) || (!strcasecmp(wname, "rbior1.1"))) {
      return 2;
    }
  }
  ...
  cudaMemcpyToSymbol(c_kern_L, f1_l, hlen*sizeof(DTYPE), 0,
      cudaMemcpyHostToDevice);
  cudaMemcpyToSymbol(c_kern_H, f1_h, hlen*sizeof(DTYPE), 0,
      cudaMemcpyHostToDevice);
  cudaMemcpyToSymbol(c_kern_IL, f1_il, hlen*sizeof(DTYPE), 0,
      cudaMemcpyHostToDevice);
  cudaMemcpyToSymbol(c_kern_IH, f1_ih, hlen*sizeof(DTYPE), 0,
      cudaMemcpyHostToDevice);
...
int w_set_filters_forward(DTYPE* filter1, DTYPE* filter2, uint len) {
  if (cudaMemcpyToSymbol(c_kern_L, filter1, len*sizeof(DTYPE), 0,
        cudaMemcpyHostToDevice) != cudaSuccess ||
      cudaMemcpyToSymbol(c_kern_H, filter2, len*sizeof(DTYPE), 0,
        cudaMemcpyHostToDevice) != cudaSuccess) {
    return -3;
  }
  return 0;
...
int w_set_filters_inverse(DTYPE* filter1, DTYPE* filter2, uint len) {
  if (cudaMemcpyToSymbol(c_kern_IL, filter1, len*sizeof(DTYPE), 0,
        cudaMemcpyHostToDevice) != cudaSuccess ||
      cudaMemcpyToSymbol(c_kern_IH, filter2, len*sizeof(DTYPE), 0,
        cudaMemcpyHostToDevice) != cudaSuccess)
  {
     return -3;
  }
...
int w_forward_separable_1d(DTYPE* d_image, DTYPE** d_coeffs, DTYPE* d_tmp,
  ...
  if ((levels > 1) && ((levels & 1) == 0))
    cudaMemcpy(d_coeffs[0], d_tmp, Nr*Nc2*sizeof(DTYPE),
...
// must be run with grid size = (2*Nr, 2*Nc) ; Nc = numcols of input coeffs.
// Here the param Nr is actually doubled wrt Nr_coeffs because of the vertical
// oversampling.
// pass 2 : (tmp1, tmp2)  ==> Horiz convol with IL, IH  + horiz
// oversampling ==> I
__global__ void w_kern_inverse_pass2(DTYPE* tmp1, DTYPE* tmp2, DTYPE* img,
    int Nr, int Nc, int Nc2, int hlen) {
  int gidx = threadIdx.x + blockIdx.x*blockDim.x;
  int gidy = threadIdx.y + blockIdx.y*blockDim.y;

  // horiz oversampling : Input (Nr*2, Nc) => Output (Nr*2, Nc*2)
  if (gidy < Nr && gidx < Nc2) {
    int c, hL, hR;
    int hlen2 = hlen/2; // Convolutions with even/odd indices of the kernels
    if (hlen2 & 1) { // odd half-kernel size
     ...
     // reconstruction.
       ...
       hR = c-1;
       // virtual id for shift
       // TODO : for the very first convolution (on the edges), this is not
       // exactly accurate (?)
       gidx += 1;
     }
     int jx1 = c - gidx/2;
     ...
     if ((hlen2 & 1) == 1)
       img[gidy * Nc2 + gidx] = res_1 + res_2;
     else 
       img[gidy * Nc2 + (gidx-1)] = res_1 + res_2;
  }
}



int w_inverse_separable(DTYPE* d_image, DTYPE** d_coeffs, DTYPE* d_tmp,
    w_info winfos) {
  int Nr = winfos.Nr, Nc = winfos.Nc, levels = winfos.nlevels,
      hlen = winfos.hlen;
  ...
  for (int i = levels-1; i >= 1; i--) {
    ...
    w_kern_inverse_pass1<<<n_blocks_1, n_threads_per_block>>>(d_coeffs[0],
        d_coeffs[3*i+1], d_coeffs[3*i+2], d_coeffs[3*i+3], d_tmp1, d_tmp2,
        tNr[i+1], tNc[i+1], tNr[i], hlen);
    w_kern_inverse_pass2<<<n_blocks_2, n_threads_per_block>>>(d_tmp1, d_tmp2,
        d_coeffs[0], tNr[i], tNc[i+1], tNc[i], hlen);
  }
  // First scale
  n_blocks_1 = dim3(w_iDivUp(tNc[1], tpb), w_iDivUp(tNr[0], tpb), 1);
  n_blocks_2 = dim3(w_iDivUp(Nc, tpb), w_iDivUp(Nr, tpb), 1);
  w_kern_inverse_pass1<<<n_blocks_1, n_threads_per_block>>>(d_coeffs[0],
      d_coeffs[1], d_coeffs[2], d_coeffs[3], d_tmp1, d_tmp2, tNr[1], tNc[1],
      tNr[0], hlen);
  w_kern_inverse_pass2<<<n_blocks_2, n_threads_per_block>>>(d_tmp1, d_tmp2,
      d_image, tNr[0], tNc[1], tNc[0], hlen);

  return 0;
}

// (batched) 1D inverse transform. Boils down to 2D separable transform without
// the first pass.
// TODO: consider deleting these memcpy. Simply do inverse_pass2: coeffs[i] =>
// coeffs[i+1]. coeffs are overwritten anyway.
int w_inverse_separable_1d(DTYPE* d_image, DTYPE** d_coeffs, DTYPE* d_tmp,
    w_info winfos) {
  ...
  for (int i = levels-1; i >= 1; i--) {
    ...
    w_kern_inverse_pass2<<<n_blocks, n_threads_per_block>>>(d_tmp1,
        d_coeffs[i+1], d_tmp2, Nr, tNc[i+1], tNc[i], hlen);
    w_swap_ptr(&d_tmp1, &d_tmp2);
  }
  if ((levels > 1) && ((levels & 1) == 0))
    cudaMemcpy(d_coeffs[0], d_tmp1, Nr*tNc[1]*sizeof(DTYPE),
        cudaMemcpyDeviceToDevice);
  // First scale
  n_blocks = dim3(w_iDivUp(Nc, tpb), w_iDivUp(Nr, tpb), 1);
  w_kern_inverse_pass2<<<n_blocks, n_threads_per_block>>>(d_coeffs[0],
      d_coeffs[1], d_image, Nr, tNc[1], Nc, hlen);
...
__global__ void w_kern_forward_swt_pass1(DTYPE* img, DTYPE* tmp_a1,
    DTYPE* tmp_a2, int Nr, int Nc, int hlen, int level) {
  int gidx = threadIdx.x + blockIdx.x*blockDim.x;
  int gidy = threadIdx.y + blockIdx.y*blockDim.y;
  if (gidy < Nr && gidx < Nc) {
    ...
    // Convolution with periodic boundaries extension.
    // The filters are 2-upsampled at each level :
    // [h0, h1, h2, h3] --> [h0, 0, h1, 0, h2, 0, h3, 0]
    for (int jx = 0; jx <= hR+hL; jx++) {
...
// must be run with grid size = (Nc, Nr)  where Nr = numrows of input image
// Pass 2 : (tmp_a1, tmp_a2) ==>  Vertic convolution on tmp_a1 and tmp_a2 with
// L, H  ==> (a, h, v, d)
__global__ void w_kern_forward_swt_pass2(DTYPE* tmp_a1, DTYPE* tmp_a2,
    DTYPE* c_a, DTYPE* c_h, DTYPE* c_v, DTYPE* c_d, int Nr, int Nc, int hlen,
    int level) {
  int gidx = threadIdx.x + blockIdx.x*blockDim.x;
  int gidy = threadIdx.y + blockIdx.y*blockDim.y;
  if (gidy < Nr && gidx < Nc) {
...
int w_forward_swt_separable(DTYPE* d_image, DTYPE** d_coeffs, DTYPE* d_tmp,
    w_info winfos) {
  int Nr = winfos.Nr, Nc = winfos.Nc, levels = winfos.nlevels,
    hlen = winfos.hlen;
  ...
  // First level
  w_kern_forward_swt_pass1<<<n_blocks_1, n_threads_per_block>>>(d_image,
      d_tmp1, d_tmp2, Nr, Nc, hlen, 1);
  w_kern_forward_swt_pass2<<<n_blocks_2, n_threads_per_block>>>(d_tmp1, d_tmp2,
      d_coeffs[0], d_coeffs[1], d_coeffs[2], d_coeffs[3], Nr, Nc, hlen, 1);
  
  // Other levels
  for (int i=1; i < levels; i++) {
    w_kern_forward_swt_pass1<<<n_blocks_1, n_threads_per_block>>>(d_coeffs[0],
        d_tmp1, d_tmp2, Nr, Nc, hlen, i+1);
    w_kern_forward_swt_pass2<<<n_blocks_2, n_threads_per_block>>>(d_tmp1,
        d_tmp2, d_coeffs[0], d_coeffs[3*i+1], d_coeffs[3*i+2], d_coeffs[3*i+3],
        Nr, Nc, hlen, i+1);
  }
  return 0;
}
...
// (batched) 1D forward SWT. Boils down to 2D non-separable transform without
// the second pass.
int w_forward_swt_separable_1d(DTYPE* d_image, DTYPE** d_coeffs, DTYPE* d_tmp,
    w_info winfos) {
  int Nr = winfos.Nr, Nc = winfos.Nc, levels = winfos.nlevels,
    hlen = winfos.hlen;
  ...
  // First level
  w_kern_forward_swt_pass1<<<n_blocks, n_threads_per_block>>>(d_image,
      d_coeffs[0], d_coeffs[1], Nr, Nc, hlen, 1);
  // Other levels
  for (int i=1; i < levels; i++) {
    w_kern_forward_swt_pass1<<<n_blocks, n_threads_per_block>>>(d_tmp1, d_tmp2,
        d_coeffs[i+1], Nr, Nc, hlen, i+1);
    w_swap_ptr(&d_tmp1, &d_tmp2);
  }
  if ((levels & 1) == 0)
...
__global__ void w_kern_inverse_swt_pass1(DTYPE* c_a, DTYPE* c_h, DTYPE* c_v,
    DTYPE* c_d, DTYPE* tmp1, DTYPE* tmp2, int Nr, int Nc, int hlen,
    int level) {
  int gidx = threadIdx.x + blockIdx.x*blockDim.x;
  int gidy = threadIdx.y + blockIdx.y*blockDim.y;
  // vertic oversampling : Input (Nr, Nc) => Output (Nr*2, Nc)
...
__global__ void w_kern_inverse_swt_pass2(DTYPE* tmp1, DTYPE* tmp2, DTYPE* img,
    int Nr, int Nc, int hlen, int level) {
  int gidx = threadIdx.x + blockIdx.x*blockDim.x;
  int gidy = threadIdx.y + blockIdx.y*blockDim.y;
  // horiz oversampling : Input (Nr*2, Nc) => Output (Nr*2, Nc*2)
  if (gidy < Nr && gidx < Nc) {
    ...
    // even half-kernel size : center is shifted to the RIGHT for reconstruction.
    else {
      c = hlen/2 - 0;
      hL = c;
      hR = c-1;
...
int w_inverse_swt_separable(DTYPE* d_image, DTYPE** d_coeffs, DTYPE* d_tmp,
    w_info winfos) {
  ...
  for (int i = levels-1; i >= 1; i--) {
    w_kern_inverse_swt_pass1<<<n_blocks_1, n_threads_per_block>>>(d_coeffs[0],
        d_coeffs[3*i+1], d_coeffs[3*i+2], d_coeffs[3*i+3], d_tmp1, d_tmp2, Nr,
        Nc, hlen, i+1);
    w_kern_inverse_swt_pass2<<<n_blocks_2, n_threads_per_block>>>(d_tmp1,
        d_tmp2, d_coeffs[0], Nr, Nc, hlen, i+1);
  }
  // First scale
  w_kern_inverse_swt_pass1<<<n_blocks_1, n_threads_per_block>>>(d_coeffs[0],
      d_coeffs[1], d_coeffs[2], d_coeffs[3], d_tmp1, d_tmp2, Nr, Nc, hlen, 1);
  w_kern_inverse_swt_pass2<<<n_blocks_2, n_threads_per_block>>>(d_tmp1, d_tmp2,
      d_image, Nr, Nc, hlen, 1);

  return 0;
}


// (batched) 1D inverse SWT. Boils down to 2D non-separable transform without
// the first pass.
int w_inverse_swt_separable_1d(DTYPE* d_image, DTYPE** d_coeffs, DTYPE* d_tmp,
    w_info winfos) {
  ...
  for (int i = levels-1; i >= 1; i--) {
    w_kern_inverse_swt_pass2<<<n_blocks, n_threads_per_block>>>(d_tmp1,
        d_coeffs[i+1], d_tmp2, Nr, Nc, hlen, i+1);
    w_swap_ptr(&d_tmp1, &d_tmp2);
  }
  if ((levels & 1) == 0)
    cudaMemcpy(d_coeffs[0], d_tmp, Nr*Nc*sizeof(DTYPE), cudaMemcpyDeviceToDevice);
  // First scale
  w_kern_inverse_swt_pass2<<<n_blocks, n_threads_per_block>>>(d_coeffs[0],
      d_coeffs[1], d_image, Nr, Nc, hlen, 1);
